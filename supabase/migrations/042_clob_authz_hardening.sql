-- =====================================================================
-- Migration 042: CLOB authorization hardening (audit finding #1, CRITICAL)
--
-- clob_place_order / clob_cancel_order are SECURITY DEFINER and were EXECUTE-
-- granted to anon + authenticated while trusting a caller-supplied p_user_id
-- with no tie to the authenticated principal. That let ANY caller (even anon)
-- invoke them directly via PostgREST and trade / spend / cancel as ANY user.
--
-- Fix (defense in depth):
--   1. In-function guard: in an end-user session (auth.uid() present) the caller
--      may only act as themselves (SQLSTATE P0121). Server API calls run as
--      service_role (auth.uid() NULL) and are unaffected.
--   2. REVOKE EXECUTE from PUBLIC/anon/authenticated; GRANT only to service_role
--      (the role the server API uses). clob_get_book stays public (aggregated,
--      no identity leak) so order-book depth reads keep working.
--
-- Idempotent: CREATE OR REPLACE + REVOKE/GRANT only. No data changes. Bodies are
-- the deployed migration-033 bodies with only the guard added (zero drift).
-- =====================================================================
...
CREATE OR REPLACE FUNCTION public.clob_cancel_order(p_user_id uuid, p_order_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- ---------------------------------------------------------------------
-- Lock down EXECUTE: only the server (service_role) + owner/superuser may call
-- the state-mutating matching / cancel RPCs.
-- ---------------------------------------------------------------------
REVOKE EXECUTE ON FUNCTION public.clob_place_order(uuid, uuid, uuid, public.order_side, public.clob_action, public.order_type, numeric, numeric, public.currency_code, text, timestamptz) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.clob_place_order(uuid, uuid, uuid, public.order_side, public.clob_action, public.order_type, numeric, numeric, public.currency_code, text, timestamptz) FROM anon;
REVOKE EXECUTE ON FUNCTION public.clob_place_order(uuid, uuid, uuid, public.order_side, public.clob_action, public.order_type, numeric, numeric, public.currency_code, text, timestamptz) FROM authenticated;
REVOKE EXECUTE ON FUNCTION public.clob_cancel_order(uuid, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.clob_cancel_order(uuid, uuid) FROM anon;
REVOKE EXECUTE ON FUNCTION public.clob_cancel_order(uuid, uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.clob_place_order(uuid, uuid, uuid, public.order_side, public.clob_action, public.order_type, numeric, numeric, public.currency_code, text, timestamptz) TO service_role;
GRANT EXECUTE ON FUNCTION public.clob_cancel_order(uuid, uuid) TO service_role;
