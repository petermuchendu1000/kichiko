-- =====================================================================
-- Migration 045: CLOB cash-conservation rounding fix (audit finding #7, MED/LOW)
--
-- wallets.available_balance / reserved_balance are numeric(20,6), but every CLOB
-- wallet movement was rounded to 2 decimals (ROUND(x / rate, 2)). Positions,
-- shares and notional track at 6 dp, so each fill / escrow op leaked up to ~half
-- a cent between cash and collateral. A 4000-op fuzz drifted cash+collateral
-- from the invariant baseline by ~$0.27 (bounded per fill, unbounded in
-- aggregate) -- a ledger-conservation defect.
--
-- Fix: round wallet movements to 6 dp (the wallet column scale) instead of 2,
-- and widen clob_orders.reserved_usd from numeric(20,2) to numeric(20,6) so
-- reserve-in and release-out are symmetric at the same precision. This is the
-- finest conservation the schema allows (6 dp columns); residual is <= 1e-6.
--
-- CREATE OR REPLACE preserves the migration-042/043 grants + P0121 guard and the
-- 043 budget-cap / cost-basis logic (bodies below are the deployed versions with
-- only the rounding scale changed). Additive/reversible.
-- =====================================================================

BEGIN;

-- Widen escrow precision to match the 6 dp wallet columns (lossless widening).
ALTER TABLE public.clob_orders ALTER COLUMN reserved_usd TYPE numeric(20,6);
...
BEGIN
  ...
  IF v_order.action = 'buy' THEN
    ...
    v_loc := ROUND(v_order.reserved_usd / v_order.exchange_rate_to_usd, 6);
    UPDATE public.wallets SET
