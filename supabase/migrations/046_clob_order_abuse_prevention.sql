-- 046_clob_order_abuse_prevention.sql
-- Milestone 6 (audit recommendation): per-user abuse prevention for the CLOB.
--
-- Adds two defenses inside clob_place_order (unchanged signature, so CREATE OR
-- REPLACE preserves the migration-042/043 grant lockdown):
--   1. Open-order caps  -- global (250/user) and per-market (60/user/market),
--      bounding order-book flooding that would bloat the matching ladder.
--   2. Placement rate limit -- 100 new orders / rolling 10s per user, bounding
--      RPC spam / DoS.
-- New SQLSTATEs: P0130 (global cap), P0131 (per-market cap), P0132 (rate limit),
-- surfaced by the API as HTTP 429. Thresholds sit well above observed
-- legitimate usage (74 open globally / 10 per market / 49 per 10s) so no real
-- market-maker is affected.
--
-- A supporting partial index keeps the open-order counts index-only and cheap.

CREATE INDEX IF NOT EXISTS idx_clob_orders_user_open
  ON public.clob_orders (user_id, market_id)
